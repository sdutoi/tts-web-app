...
export default function StoryAssistant() {
  ...
  return (
    <main className="max-w-3xl mx-auto p-6 space-y-6">
      <h1 className="text-2xl font-semibold">Story assistant</h1>

      <section className="space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <label className="flex items-center gap-2"><span className="w-24">Language</span>
          ...
          <label className="flex items-center gap-2"><span className="w-24">Level</span>
          ...
          <label className="flex items-center gap-2"><span className="w-24">Words</span>
        ...
        <input className="w-full border rounded p-2" placeholder="Interests / themes (e.g., travel, cooking, sci-fi)" value={interests} onChange={(e)=>setInterests(e.target.value)} />
        <input className="w-full border rounded p-2" placeholder="Optional sentence to include (in target language)" value={sentence} onChange={(e)=>setSentence(e.target.value)} />
        <input className="w-full border rounded p-2" placeholder="Optional style/tone (e.g., funny, dramatic)" value={style} onChange={(e)=>setStyle(e.target.value)} />
        <div className="flex gap-2">
          <button className="px-4 py-2 rounded bg-blue-600 text-white disabled:opacity-50" onClick={()=>callStory("generate")} disabled={genLoading}>Generate story</button>
          <button className="px-4 py-2 rounded bg-emerald-600 text-white disabled:opacity-50" onClick={()=>callStory("refine")} disabled={genLoading || !storyJson}>Refine story</button>
        </div>
      ...
      {storyJson && (
        <section className="space-y-3">
          <h2 className="text-xl font-semibold">{storyJson.title || "Mini Story"}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            <label className="flex items-center gap-2"><span className="w-20">Speed</span>
            ...
            <label className="flex items-center gap-2"><span className="w-20">Format</span>
            ...
            <button className="px-4 py-2 rounded bg-indigo-600 text-white disabled:opacity-50" onClick={tts} disabled={!storyJson?.story}>Generate audio</button>
          </div>
          <audio ref={audioRef} controls className="w-full" />
          {audioUrl && (
            <a href={audioUrl} download={`story.${format}`} className="text-blue-700 underline">Download audio</a>
          )}
        </section>
      )}
    </main>
